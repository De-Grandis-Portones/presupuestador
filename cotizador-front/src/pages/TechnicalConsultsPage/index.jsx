...
export default function TechnicalConsultsPage() {
  ...
  // "Abrir consulta técnica"): ?ref=NV4304 precarga el numero y abre el formulario
  // de "nuevo ticket" solo, sin que el usuario tenga que ir a buscarlo el mismo.
  useEffect(() => {
    const ref = searchParams.get("ref");
    if (!ref) return;
    setReferenceNumber(ref);
    if (isTechnical) setShowNewForm(true);
    ...
    onSuccess: (result) => {
      ...
      if (result?.bulk) {
        ...
        if (first) {
          ...
          qc.setQueryData(["technicalConsult", first.id], first);
        }
      } else {
        setBulkNotice("");
        setSelectedId(result.id);
        qc.setQueryData(["technicalConsult", result.id], result);
      }
    },
  });

  const replyM = useMutation({
    mutationFn: () => addTechnicalConsultMessage(selectedId, { message: replyMessage, attachment: replyAttachment }),
    onSuccess: (ticket) => {
      setReplyMessage("");
  ...
  const closeM = useMutation({
    mutationFn: () => closeTechnicalConsult(selectedId, { resolution: resolutionText, attachment: resolutionAttachment }),
    onSuccess: (ticket) => {
      setResolutionText("");
