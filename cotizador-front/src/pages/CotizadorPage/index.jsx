...
export default function CotizadorPage({ catalogKind = "porton" }) {
  ...
  const confirmReturnedM = useMutation({ mutationFn: async () => { const payload = getDraftPayload(); validateConfirm(payload); if (!quoteId) throw new Error("Quote inválida"); await updateQuote(quoteId, payload); return await confirmReturnedMeasurementQuote(quoteId); }, onSuccess: async (res) => { await qc.invalidateQueries({ queryKey: ["quote", quoteId] }); await qc.invalidateQueries({ queryKey: ["quotes", "mine"] }); navigate("/menu", { replace: true }); toast.success("Se envió a Comercial para su aprobación."); const absorbed = Number(res?.tolerance_metrics?.surface_absorbed_amount || 0); if (absorbed > 0) { const diffM2 = res.tolerance_metrics.surface_absorbed_diff_m2; toast.success(`Diferencia de medición de ${diffM2} m² dentro de tolerancia: se absorbió $${absorbed.toLocaleString("es-AR")}, no se le cobra al cliente.`, { duration: 8000 }); } }, onError: (e) => toast.error(e?.message || "No se pudo enviar a Comercial") });
