...
function replaceExact(content, from, to, label) {
  ...
  if (!content.includes(from)) {
    console.warn(`[portones-parantes] No se encontro bloque para ${label}; se omite.`);
    return content;
  }
  return content.replace(from, to);
}
function replaceRegex(content, regex, to, label) {
  if (typeof to === "string" && content.includes(to)) return content;
  if (!regex.test(content)) {
    console.warn(`[portones-parantes] No se encontro patron para ${label}; se omite.`);
    return content;
  }
...
function insertAfter(content, anchor, insertion, label) {
  ...
  if (!content.includes(anchor)) {
    console.warn(`[portones-parantes] No se encontro ancla para ${label}; se omite.`);
    return content;
  }
...
function patchStore() {
  ...
  content = content.replaceAll("l.qty > 0 || l.previously_billed_line || isIntegerQtyProductId(l.product_id)", "l.qty > 0 || l.previously_billed_line || isIntegerQtyLine(l)");
...
function patchPortonDimensionsDistance() {
  ...
  content = replaceExact(
    ...
    `    if (!String(dimensions?.parantes_referencia_lado || "").trim()) patch.parantes_referencia_lado = "izquierdo";`,
    "PortonDimensions no forzar 800 mientras se edita",
  );
  content = replaceExact(
    content,
    `                        value={aptoReferenciaDistancia || "800"}`,
    `                        value={aptoReferenciaDistancia}`,
    "PortonDimensions value editable distancia fija",
  );
  write(rel, content);
}
...
try {
  ...
  patchCotizadorPage();
  patchDashboard();
  patchPortonDimensionsDistance();
  console.log("[portones-parantes] Patch aplicado.");
} catch (err) {
  console.error("[portones-parantes] Error aplicando patch:", err);
  process.exit(1);
}
